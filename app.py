...
def strategy_bonus(player, strategy, round_no):
    pos=player["pos"]
    if strategy=="rb-wr-heavy":
        if pos in {"RB", "WR"} and round_no <= 6: return 5
        if pos in {"QB", "TE"} and round_no <= 3: return -8
    if strategy=="zero-rb":
...
def mock_strategy_score(player, strategy, round_no, roster_counts):
    ...
    if strategy == "rb-wr-heavy":
        ...
        if core_count < 3:
            if pos in {"RB", "WR"}: score += 12
            if pos in {"QB", "TE"}: score -= 15
        elif pos in {"RB", "WR"} and round_no <= 10:
            score += 4
        if pos == "QB" and roster_counts.get("QB", 0) >= 2: score -= 20
        if pos == "TE" and roster_counts.get("TE", 0) >= 2: score -= 16
    ...
    targets = {"QB":2,"RB":4,"WR":5,"TE":2} if strategy == "rb-wr-heavy" else {"QB":1,"RB":4,"WR":5,"TE":1}
...
def _yahoo_credentials():
    return {
        ...
        "app_id": str(os.getenv("YAHOO_APP_ID", "") or "").strip(),
    }


def _yahoo_client_fingerprint():
    """Safe identifier used only to detect when Render was switched to a new Yahoo app."""
    client_id = _yahoo_credentials().get("client_id", "")
    return hashlib.sha256(client_id.encode("utf-8")).hexdigest()[:12] if client_id else ""
...
def _yahoo_token():
    ...
    for path in _yahoo_token_paths():
        ...
        if data.get("access_token") or data.get("refresh_token"):
            token_fp = str(data.get("client_id_fingerprint") or "")
            current_fp = _yahoo_client_fingerprint()
            if token_fp and current_fp and token_fp != current_fp:
                app.logger.warning("Ignoring Yahoo OAuth token created for a different Yahoo Client ID.")
                continue
    ...
    if str(request.args.get("force") or "").lower() in {"1", "true", "yes"}:
        for path in _yahoo_token_paths():
            try:
                ...
                app.logger.exception("Unable to clear old Yahoo token %s", path)
        try:
            if YAHOO_PREF_FILE.exists():
                YAHOO_PREF_FILE.unlink()
        except Exception:
...
def yahoo_sync():
    if not _yahoo_token():
        return jsonify(
            ok=False,
            error="Yahoo is not signed in.",
            detail="Click Sign In / Reconnect Yahoo first. After the Yahoo badge says Connected, choose a league and sync.",
        ), 401
    selected = _yahoo_selected_league()
    if not selected:
        try:
            ...
            return jsonify(ok=False, error="Yahoo is signed in, but Gridiron IQ could not load your leagues.", detail=str(exc)), 400
...
def _manual_mock_list(limit=30):
    ...
    for mock in store.values():
        if _manual_ensure_complete_pool(mock):
            changed = True
        if (mock.get("grade") or {}).get("grade_version") != "role-adjusted-v7":
...
def _build_dynamic_mock_pool(context):
    ...
    for key, stats in stats_map.items():
        ...
        # Carry usage/role signals into the draft pool so pick grading can
        # evaluate a late-round starter or WR2/RB2 on his actual opportunity,
        # not simply on how early he was drafted.
        for usage_field in (
            "games", "snap_share", "target_share", "offense_snaps",
            "targets", "receptions", "carries", "rushing_attempts",
            "attempts", "fantasy_points_ppr", "fantasy_points",
        ):
            if stats.get(usage_field) not in (None, ""):
                item[usage_field] = stats.get(usage_field)
...
def _manual_ensure_complete_pool(mock):
    ...
    changed = False
    if mock.get("formula_version") != "smart-draft-v7":
        mock["formula_version"] = "smart-draft-v7"
        changed = True
    needs_pool_upgrade = (
...
def _manual_projection_grade(player, mock=None, overall=None):
    """Score 2026 point potential relative to players normally available nearby.

    The old absolute position scale made later-round picks drift downward even
    when they were excellent choices for that part of the draft. v7 compares a
    player with same-position peers in the surrounding draft window. A strong
    Round 11 WR2 can therefore earn the same scoring grade as a strong Round 3
    player if he clearly out-produces the alternatives available around him.
    """
    ...
    if isinstance(mock, dict) and overall:
        peers = []
        center = max(1.0, float(overall))
        for peer in (mock.get("player_pool") or []):
            if str(peer.get("pos") or "").upper() != position:
                continue
            try:
                peer_projection = float(peer.get("projection", 0) or 0)
                peer_adp = float(peer.get("adp") or peer.get("rank") or 999)
            except (TypeError, ValueError):
                continue
            if peer_projection <= 0 or not 0 < peer_adp < 999:
                continue
            # Wider after the current pick than before it because the relevant
            # question is what scoring upside could reasonably be found here or
            # shortly afterward.
            if center - 24 <= peer_adp <= center + 42:
                peers.append(peer_projection)
        if len(peers) >= 5:
            below = sum(1 for value in peers if value < projection)
            equal = sum(1 for value in peers if value == projection)
            percentile = (below + 0.5 * equal) / max(1, len(peers))
            return max(45.0, min(98.0, 48.0 + percentile * 50.0))

    # Fallback only when the player pool cannot provide a useful local window.
    low_high = {
        "QB": (180.0, 390.0), "RB": (65.0, 330.0), "WR": (65.0, 330.0),
        "TE": (40.0, 260.0), "K": (75.0, 175.0), "DEF": (70.0, 180.0),
    }
    low, high = low_high.get(position, (60.0, 300.0))
    grade = 30.0 + ((projection - low) / max(1.0, high - low)) * 65.0
    return max(20.0, min(98.0, grade))


def _manual_history_grade(position, points):
    """Production context; rookies/unknown history remain neutral."""
    try:
    ...
    if points <= 0:
        return 65.0
    low_high = {
        "QB": (140.0, 420.0), "RB": (45.0, 380.0), "WR": (45.0, 380.0),
        "TE": (25.0, 260.0), "K": (65.0, 185.0), "DEF": (60.0, 190.0),
    }
    low, high = low_high.get(str(position or "").upper(), (40.0, 350.0))
    grade = 30.0 + ((points - low) / max(1.0, high - low)) * 65.0
    return max(15.0, min(98.0, grade))


def _manual_fraction(value):
    try:
...
def _manual_usage_role_grade(player):
    """Return a current-role label and usage grade without any round penalty."""
    position = str(player.get("pos") or "").upper()
    try:
        ...
        depth = 0
    try:
        games = max(1.0, float(player.get("games") or 0))
    except (TypeError, ValueError):
        games = 1.0
    snap_share = _manual_fraction(player.get("snap_share"))
    target_share = _manual_fraction(player.get("target_share"))
    try:
        carries = float(player.get("carries") or player.get("rushing_attempts") or 0)
    except (TypeError, ValueError):
        carries = 0.0
    try:
        targets = float(player.get("targets") or 0)
    except (TypeError, ValueError):
        targets = 0.0
    touches_per_game = (carries + targets) / games if games > 0 else 0.0

    role = f"{position} depth unknown" if position else "Role unknown"
    base = 65.0
    ...
    if position == "WR":
        if depth == 1:
            ...
            role, base = "WR3 / slot", 74.0
        elif depth >= 4:
            role, base = "WR rotation", 55.0
        elif target_share >= .22:
            role, base = "WR1-type usage", 92.0
        elif target_share >= .15:
            role, base = "WR2-type usage", 84.0
        elif target_share >= .10:
            role, base = "WR3-type usage", 70.0
    elif position == "RB":
        if depth == 1:
            role, base = "RB1", 95.0
        elif depth == 2:
            role, base = "RB2 / committee", 84.0
        elif depth == 3:
            role, base = "RB3 / change of pace", 67.0
        elif depth >= 4:
            role, base = "RB reserve", 48.0
        elif touches_per_game >= 15:
            role, base = "RB1-type workload", 92.0
        elif touches_per_game >= 9:
            role, base = "RB2-type workload", 82.0
        elif touches_per_game >= 5:
            role, base = "Committee workload", 68.0
    elif position == "TE":
        if depth == 1:
            role, base = "TE1", 92.0
        elif depth == 2:
            ...
            role, base = "TE reserve", 52.0
        elif target_share >= .14:
            role, base = "Starting TE usage", 88.0
        elif target_share >= .08:
            role, base = "Rotational TE usage", 68.0
    elif position == "QB":
        if depth == 1:
            role, base = "Starting QB", 97.0
        elif depth >= 2:
            role, base = "Backup QB", 28.0
        else:
            try:
                attempts = float(player.get("attempts") or 0)
            except (TypeError, ValueError):
                attempts = 0.0
            if attempts >= 300:
                role, base = "Starting QB usage", 92.0
            else:
                role, base = "QB role unknown", 68.0
    elif position == "K":
        role, base = "Starting K", 80.0
    elif position == "DEF":
        role, base = "Starting D/ST", 80.0

    adjustment = 0.0
    if position in {"WR", "TE"}:
        if target_share >= .22:
            adjustment += 7
        elif target_share >= .16:
            adjustment += 4
        elif target_share >= .10:
            adjustment += 2
        if snap_share >= .75:
        ...
        if touches_per_game >= 18:
            adjustment += 7
        elif touches_per_game >= 13:
            adjustment += 5
        elif touches_per_game >= 8:
            adjustment += 2
        if snap_share >= .60:
            adjustment += 3
...
def _manual_pick_score_components(mock, player, counts=None, overall=None, round_no=None):
    ...
    try:
        adp = float(player.get("draft_adp") or player.get("adp") or player.get("rank") or 999)
    except (TypeError, ValueError):
        adp = 999.0
    if not 0 < adp < 999:
        try:
    ...
    reach_penalty = reach * 1.05 + max(0.0, reach - 10.0) * 0.55 + max(0.0, reach - 24.0) * 0.70
    ...
    scoring_grade = _manual_projection_grade(player, mock=mock, overall=overall)
    role_label, usage_grade = _manual_usage_role_grade(player)
    historical_points = player.get("fantasy_points_ppr") or player.get("fantasy_points") or 0
    history_grade = _manual_history_grade(position, historical_points)
    # Player quality is intentionally round-neutral: scoring upside, current
    # role/usage and proven production. It does not contain pick number.
    quality_grade = max(20.0, min(99.0,
        scoring_grade * 0.45 + usage_grade * 0.35 + history_grade * 0.20
    ))

    # v7 pick score has NO round multiplier. Late-round picks can earn elite
    # grades when they are high-quality, productive, useful players at a good
    # market price with a meaningful NFL role.
    score = (
        market_grade * 0.20
        + quality_grade * 0.25
        + scoring_grade * 0.25
        + roster_fit_grade * 0.15
        + usage_grade * 0.15
    )
    ...
    return {
        ...
        "quality_grade": round(quality_grade),
        "projection_grade": round(scoring_grade),
        "scoring_grade": round(scoring_grade),
        "roster_fit_grade": round(roster_fit_grade),
        "usage_grade": round(usage_grade),
        "role_label": role_label,
        "adp": round(adp, 1) if adp < 999 else 999,
        "reach": round(reach, 1),
        "value": round(value, 1),
        "severe_reach": bool(reach >= 28),
    }
...
def _manual_pick_report(mock):
    ...
    for p in roster:
        ...
        reports.append({
            ...
            "quality_grade": components.get("quality_grade", 0),
            "projection_grade": components["projection_grade"],
            "scoring_grade": components.get("scoring_grade", components["projection_grade"]),
            "roster_fit_grade": components["roster_fit_grade"],
            "usage_grade": components.get("usage_grade", 0),
            "role_label": components.get("role_label", "Role unknown"),
...
def _manual_grade(mock):
    ...
    if not roster:
        return {
            "grade_version": "role-adjusted-v7", "overall": 0,
            "projection_score": 0, "scoring_score": 0, "quality_score": 0,
            "usage_score": 0, "stats_score": 0, "value_score": 0,
            "balance_score": 0, "need_score": 0, "depth_score": 0,
            "pick_quality_score": 0, "worst_pick_score": 0,
            "reach_count": 0, "severe_reach_count": 0,
            "pick_reports": [], "projected_points": 0,
            "previous_year_points": 0, "summary": "No user picks yet.",
        }
    ...
    # Every selection counts equally. There is deliberately NO early-round or
    # late-round multiplier in v7.
    report_count = max(1, len(reports))
    def average(field):
        return round(sum(float(report.get(field, 0) or 0) for report in reports) / report_count)

    pick_quality_score = average("pick_grade")
    value_score = average("market_grade")
    projection_score = average("scoring_grade")
    scoring_score = projection_score
    quality_score = average("quality_grade")
    need_score = average("roster_fit_grade")
    usage_score = average("usage_grade")
    stats_score = average("history_grade")
    ...
    reach_count = sum(1 for report in reports if report["reach"] >= 10)
    severe_reach_count = sum(1 for report in reports if report["severe_reach"])

    # Round-neutral overall grade. Late-round picks can improve the grade just
    # as much as early picks when they add quality, scoring upside and usage.
    overall = (
        value_score * 0.20
        + quality_score * 0.20
        + scoring_score * 0.20
    ...
    overall -= max(0, 30 - worst_pick_score) * 0.04
    overall -= severe_reach_count * 0.50
    overall = round(max(0, min(99, overall)))

    strengths, concerns = [], []
    if value_score >= 85: strengths.append("strong ADP value")
    if quality_score >= 84: strengths.append("high player quality")
    if scoring_score >= 84: strengths.append("strong scoring upside for the draft ranges used")
    if need_score >= 84: strengths.append("drafted well for roster needs")
    if usage_score >= 84: strengths.append("players with strong NFL roles and usage")
    if balance_score >= 85: strengths.append("balanced final roster construction")
    if counts.get("RB", 0) < 3: concerns.append("running-back depth")
    if counts.get("WR", 0) < 4: concerns.append("wide-receiver depth")
    if counts.get("QB", 0) == 0: concerns.append("quarterback still open")
    if counts.get("TE", 0) == 0: concerns.append("tight end still open")
    if scoring_score < 65: concerns.append("limited scoring upside versus players available in the same ranges")
    if usage_score < 65: concerns.append("too many low-usage or reserve-role players")
    if need_score < 65: concerns.append("too many picks that did not address roster needs")
    if reach_count: concerns.append(f"{reach_count} pick{'s' if reach_count != 1 else ''} taken materially ahead of ADP")
    if severe_reach_count: concerns.append(f"{severe_reach_count} severe reach{'es' if severe_reach_count != 1 else ''}")

    summary = f"Overall draft grade: {overall}/100. Every round is graded on the same scale."
    if strengths: summary += " Strengths: " + ", ".join(strengths) + "."
    if concerns: summary += " Watch: " + ", ".join(concerns) + "."

    return {
        "grade_version": "role-adjusted-v7", "overall": overall,
        "projection_score": projection_score, "scoring_score": scoring_score,
        "quality_score": quality_score, "usage_score": usage_score,
        "stats_score": stats_score, "value_score": value_score,
        "balance_score": balance_score, "need_score": need_score,
        "depth_score": depth_score, "pick_quality_score": pick_quality_score,
        "worst_pick_score": worst_pick_score, "reach_count": reach_count,
        "severe_reach_count": severe_reach_count, "pick_reports": reports,
        "projected_points": projected_points, "previous_year_points": previous_points,
        "summary": summary,
    }
...
def _mock_league_profile(mock):
    ...
    requirements = _mock_requirements_for_mock(mock)
    strategy = str(mock.get("draft_strategy") or "rb-wr-heavy")
    profile.update({
...
def _mock_allowed_positions(roster, round_no, total_rounds, requirements=None, scoring="", strategy="balanced"):
    ...
    if strategy_key == "rb-wr-heavy" and requirements:
        ...
        if deficit_total and remaining_picks <= deficit_total + 1:
            target_allowed = {pos for pos, amount in target_deficits.items() if amount > 0}
            if target_allowed:
...
def manual_mock_start():
    ...
    strategy = str(data.get("draft_strategy") or "rb-wr-heavy").lower().replace("_", "-")
    allowed_strategies = {"rb-wr-heavy", "balanced", "best-player", "hero-rb", "zero-rb", "robust-rb", "late-qb"}
    if strategy not in allowed_strategies:
        strategy = "rb-wr-heavy"
...
def manual_mock_state(mock_id):
    ...
    return jsonify(
        ...
        formula_version=mock.get("formula_version") or "smart-draft-v7",
        server_build=MOCK_DRAFT_BUILD,
...
def _injury_expected_return(master, injury_row=None, news_items=None, status="", description=""):
    ...
    structured_fields = (
        "expected_return", "expected_return_date", "return_date", "estimated_return",
        "estimated_return_date", "return_week", "expected_return_week",
        "eligible_to_return", "eligible_return_date", "recovery_timeline", "timeline",
    )
    for source_name, source_row in (("Live injury report", injury_row), ("Current player database", master)):
        for key in structured_fields:
            value = source_row.get(key)
            if value not in (None, ""):
                label = str(value).strip()
                if key.endswith("week") and label.isdigit():
                    label = f"Week {label}"
                return {
                    "expected_return": label,
                    "return_confidence": "Reported",
                    "return_source": source_name,
                }

    text_candidates = []
    for source_name, value in (
        ("Live injury report", description),
        ("Live injury report", injury_row.get("note")),
        ("Live injury report", injury_row.get("description")),
        ("Current player database", master.get("practice_description")),
        ("Current player database", master.get("injury_description")),
    ):
        if value:
            text_candidates.append((source_name, str(value)))

    for item in news_items or []:
        if not isinstance(item, dict):
            continue
        combined = " ".join(str(item.get(k) or "") for k in ("title", "summary", "description"))
        if combined.strip():
    ...
    return {
        "expected_return": "No timetable announced" if active else "Not applicable",
...
def _player_research_injury(player_name, news_items=None):
    ...
    result = {
        ...
        "description": master.get("practice_description") or master.get("injury_description") or "",
        "updated_at": "",
    ...
    try:
        ...
        for row in rows:
            if not _player_news_matches(row, player_name, fp_id):
                continue
            live_row = row if isinstance(row, dict) else {}
            result = {
                ...
                "description": row.get("description") or row.get("note") or result["description"],
...
# v53 Player Research Auto News -------------------------------------------------
# The table performs one shared live NFL-news feed check every 10 minutes instead
...
@app.post("/api/player-research/news-scan")
def player_research_news_scan_api():
    payload = request.get_json(silent=True) or {}
    raw_players = payload.get("players") or []
    if not isinstance(raw_players, list):
        return jsonify(ok=False, error="players must be a list."), 400

    # The Player Research page currently contains roughly 1,300 rows. Keep a
    # generous hard ceiling to prevent accidental oversized requests.
    players = []
    seen = set()
    for value in raw_players[:1600]:
        name = str(value or "").strip()
        if not name or len(name) > 100:
            continue
        norm = _pr_identity_key(name)
        if not norm or norm in seen:
            continue
        seen.add(norm)
        players.append((name, norm))

    feed = _player_news_live_feed()
    feed_articles = feed.get("articles", []) if isinstance(feed, dict) else []
    matches = {}
    cutoff = (datetime.now(timezone.utc) - timedelta(days=7)).timestamp()
    ...
    for name, norm in players:
        ...
        for article in feed_articles:
            ...
            if published_ts and published_ts < cutoff:
                continue
            player_news.append({key: article.get(key) for key in (
                "title", "summary", "published", "source", "category", "url"
            )})
        if player_news:
...
def _strength_of_schedule_map():
    """Build position-adjusted 2026 schedule difficulty for fantasy players."""
    payload = _defensive_stats_snapshot()
    teams = payload.get("teams", {}) or {}
    schedule = payload.get("schedule_2026", []) or []
    signature = (
        payload.get("updated_at"), payload.get("season"),
        payload.get("matchup_season"), len(teams), len(schedule),
    )
    if _SOS_CACHE.get("signature") == signature:
        return _SOS_CACHE.get("data", {})
    if len(teams) < 28 or len(schedule) < 200:
        _SOS_CACHE.update(signature=signature, data={})
        return {}

    positions = ("QB", "RB", "WR", "TE")
    team_codes = sorted(_def_team_code(team) for team in teams)

    # Convert positional PPR allowance into a 0-100 defense-difficulty input.
    # A defense allowing fewer PPR points per target receives a higher grade.
    allowance_values = {}
    for position in ("RB", "WR", "TE"):
        values = []
        for team in team_codes:
            allowed = ((teams.get(team) or {}).get("allowed_by_position") or {}).get(position) or {}
            value = allowed.get("ppr_per_target")
            if value is not None and _off_num(value) > 0:
                values.append(_off_num(value))
        allowance_values[position] = sorted(values)
    ...
    def percentile(values, value):
        ...
        return (below + max(0, equal - 1) / 2) / (len(values) - 1) * 100

    def defense_difficulty(team, position):
        defense = teams.get(_def_team_code(team), {}) or {}
        secondary = _off_num(defense.get("secondary_grade"), 50)
        pass_rush = _off_num(defense.get("pass_rush_grade"), 50)
        run_defense = _off_num(defense.get("run_defense_grade"), 50)
        front = _off_num(defense.get("front_grade"), 50)
        if position == "QB":
            return _def_clamp(secondary * 0.62 + pass_rush * 0.38)
        if position == "RB":
            return _def_clamp(run_defense * 0.68 + front * 0.32)
        allowed = ((defense.get("allowed_by_position") or {}).get(position) or {}).get("ppr_per_target")
        allowance_difficulty = 100 - percentile(allowance_values.get(position, []), allowed)
        return _def_clamp(secondary * 0.62 + pass_rush * 0.18 + allowance_difficulty * 0.20)

    defense_grades = {
        position: {team: defense_difficulty(team, position) for team in team_codes}
        for position in positions
    }
    defense_ranks = {
        position: {
            team: rank for rank, (team, _) in enumerate(
                sorted(grades.items(), key=lambda item: (-item[1], item[0])), 1
            )
        }
        for position, grades in defense_grades.items()
    }

    games_by_team = {team: [] for team in team_codes}
    for game in schedule:
        away = _def_team_code(game.get("away_team"))
        home = _def_team_code(game.get("home_team"))
        if away in games_by_team and home in games_by_team:
            games_by_team[away].append({**game, "opponent": home, "location": "Away"})
            games_by_team[home].append({**game, "opponent": away, "location": "Home"})
    ...
    for position in positions:
        ...
        for team, games in games_by_team.items():
            grades = [defense_grades[position].get(game["opponent"], 50) for game in games]
            playoff = [
                defense_grades[position].get(game["opponent"], 50)
                for game in games if 15 <= int(game.get("week") or 0) <= 17
            ]
            averages[team] = sum(grades) / len(grades) if grades else 50
            playoff_averages[team] = sum(playoff) / len(playoff) if playoff else 50

        # Rank 1 is the easiest/most favorable fantasy schedule.
        ranks = {
            team: rank for rank, (team, _) in enumerate(
                sorted(averages.items(), key=lambda item: (item[1], item[0])), 1
            )
        }
        playoff_ranks = {
            team: rank for rank, (team, _) in enumerate(
                sorted(playoff_averages.items(), key=lambda item: (item[1], item[0])), 1
            )
        }

        for team, games in games_by_team.items():
            rank = ranks[team]
            playoff_rank = playoff_ranks[team]
            label = "Easy" if rank <= 8 else "Average" if rank <= 24 else "Hard"
            playoff_label = "Easy" if playoff_rank <= 8 else "Average" if playoff_rank <= 24 else "Hard"
            week_rows = []
            for game in sorted(games, key=lambda item: int(item.get("week") or 0)):
                opponent = game["opponent"]
                opponent_rank = defense_ranks[position].get(opponent, 16)
                matchup_label = "Hard" if opponent_rank <= 8 else "Average" if opponent_rank <= 24 else "Easy"
                week_rows.append({
                    "week": int(game.get("week") or 0),
                    "opponent": opponent,
                    "location": game.get("location") or "",
                    "gameday": game.get("gameday") or "",
                    "defense_rank": opponent_rank,
                    "defense_grade": round(defense_grades[position].get(opponent, 50), 1),
                    "matchup_score": round(99 - (opponent_rank - 1) * 98 / 31, 1),
                    "matchup_label": matchup_label,
                })
            played_weeks = {item["week"] for item in week_rows}
            bye_weeks = [week for week in range(1, 19) if week not in played_weeks]
            result[position][team] = {
                "season": int(payload.get("matchup_season") or 2026),
                "baseline_season": int(payload.get("season") or 2025),
                "position": position,
                "team": team,
                "rank": rank,
                "score": round(99 - (rank - 1) * 98 / 31, 1),
                "label": label,
                "summary": f"{_sos_ordinal(rank)} easiest {position} schedule",
                "average_opponent_grade": round(averages[team], 1),
                "playoff_rank": playoff_rank,
                "playoff_score": round(99 - (playoff_rank - 1) * 98 / 31, 1),
                "playoff_label": playoff_label,
                "playoff_summary": f"{_sos_ordinal(playoff_rank)} easiest Weeks 15-17 schedule",
                "bye_week": bye_weeks[0] if bye_weeks else None,
                "games": len(week_rows),
                "schedule": week_rows,
                "source": payload.get("source") or "Gridiron IQ defensive schedule model",
                "updated_at": payload.get("updated_at") or "",
            }

    _SOS_CACHE.update(signature=signature, data=result)
    return result


def _player_strength_of_schedule(team, position, include_schedule=False):
    position = str(position or "").upper()
    team = _def_team_code(team)
    if position not in {"QB", "RB", "WR", "TE"} or not team:
        return None
    record = (_strength_of_schedule_map().get(position) or {}).get(team)
    if not record:
        return None
    if include_schedule:
        return dict(record)
    return {key: record.get(key) for key in (
        "season", "baseline_season", "position", "team", "rank", "score",
        "label", "summary", "average_opponent_grade", "playoff_rank",
        "playoff_score", "playoff_label", "playoff_summary", "bye_week",
        "games", "updated_at",
    )}
...
def _build_pr_position_rows_uncached(position="", limit=1000, platform="ESPN"):
    ...
    for norm, player in pool.items():
        ...
        rows.append({
            ...
            "sos_summary": sos.get("summary") or "",
            "sos_playoff_rank": sos.get("playoff_rank"),
            "sos_playoff_label": sos.get("playoff_label") or "",
            "sos_bye_week": sos.get("bye_week"),
...
def _pr_profile(player_id):
    ...
    strength_of_schedule = _player_strength_of_schedule(
        bio.get("team"), bio.get("position"), include_schedule=True,
    )
    ...
    return {
        ...
        "data_notes": [
            ...
            "Strength of schedule: 2026 opponents adjusted to the player's position using the latest defensive baseline.",
        ],
    }
...
def _player_research_profile_by_name(player_name, platform="ESPN"):
    ...
    strength_of_schedule = _player_strength_of_schedule(
        row.get("team"), row.get("position"), include_schedule=True,
    )
    profile = {
        "bio": {
            key: row.get(key)
        ...
        "strength_of_schedule": strength_of_schedule,
        "trend": _pr_trend(history),
        "injury": _player_research_injury(row["name"], news_items=recent_news),
        "news": recent_news,
        ...
        "data_notes": [
            ...
            "Expected return uses reported injury timelines and recent news; when no credible window exists, Gridiron IQ shows No timetable announced.",
